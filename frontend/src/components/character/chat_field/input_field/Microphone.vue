<script setup>
// 语音输入：VAD 检测说话起止，开口即打断 AI，说完转 PCM 发后端 ASR。
...
const startRecording = async () => {
  ...
  try {
    vadInstance = await MicVAD.new({
      ...
      // 误触发（开口太短被判为非语音）：onSpeechEnd 不会执行，
      // 这里必须复位 isSpeaking，否则语音条会一直亮着停不下来。
      onVADMisfire: () => {
        isSpeaking.value = false;
      },
      ortConfig: (ort) => {
